...
const DateRangePickerConfiguration = ({ setFieldConfig, setPreviewConfig }) => {
  ...
  const handleSaveConfig = () => {
    ...
    createField({ name, type: 'DateRangePicker', config }); // Save the configuration
    ...
    <Box sx={{ position: 'relative', paddingBottom: '60px', minHeight: '89.5%', width: '60vw' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', bgcolor: 'lightyellow', py: 1, px: 2, borderRadius: 1, mb: 1 }}>
        ...
        <Typography variant="body2" sx={{ flexShrink: 1, marginLeft: '15px' }}>
          A "Date Range Picker" is used to identify a start date and an end date. If you need a single date, please use the Date instead.   The Button Group on the top row will determine the input method.  The Resolution checkbox group allows you to determine how deeply you want to track the date, from simply the year down to a second.  
        </Typography>
      </Box>
...
export default DateRangePickerConfiguration;
